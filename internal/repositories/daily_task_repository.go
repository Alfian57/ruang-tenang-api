...
type DailyTaskRepository interface {
	// Task CRUD
	CreateTask(task *models.DailyTask) error
	GetTaskByID(id uint) (*models.DailyTask, error)
	UpdateTask(task *models.DailyTask) error

	// Get tasks for a user
	GetUserTasksForDate(userID uint, date time.Time) ([]models.DailyTask, error)
	GetUserTask(userID uint, taskType models.DailyTaskType, date time.Time) (*models.DailyTask, error)

	// Batch create daily tasks
	CreateDailyTasksForUser(userID uint, date time.Time) error

	// Task progress
	IncrementTaskProgress(userID uint, taskType models.DailyTaskType, date time.Time) error
	MarkTaskCompleted(userID uint, taskType models.DailyTaskType, date time.Time) error
	...
	// Summary
	GetDailyTaskSummary(userID uint, date time.Time) (*models.DailyTaskSummary, error)

	// History
	GetTaskHistory(userID uint, limit, offset int) ([]models.DailyTaskSummary, int64, error)
}
...
func (r *dailyTaskRepository) CreateTask(task *models.DailyTask) error {
	return r.db.Create(task).Error
}

func (r *dailyTaskRepository) GetTaskByID(id uint) (*models.DailyTask, error) {
...
func (r *dailyTaskRepository) UpdateTask(task *models.DailyTask) error {
	return r.db.Save(task).Error
}

func (r *dailyTaskRepository) GetUserTasksForDate(userID uint, date time.Time) ([]models.DailyTask, error) {
...
func (r *dailyTaskRepository) GetUserTask(userID uint, taskType models.DailyTaskType, date time.Time) (*models.DailyTask, error) {
	var task models.DailyTask
	dateOnly := date.Truncate(24 * time.Hour)
...
func (r *dailyTaskRepository) CreateDailyTasksForUser(userID uint, date time.Time) error {
	...
	configs := models.GetDailyTaskConfigs()
...
func (r *dailyTaskRepository) IncrementTaskProgress(userID uint, taskType models.DailyTaskType, date time.Time) error {
	dateOnly := date.Truncate(24 * time.Hour)
	...
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				...
				config := models.GetTaskConfig(taskType)
				if config == nil {
					return nil
				}
				task = models.DailyTask{
...
func (r *dailyTaskRepository) MarkTaskCompleted(userID uint, taskType models.DailyTaskType, date time.Time) error {
	dateOnly := date.Truncate(24 * time.Hour)
...
func (r *dailyTaskRepository) GetDailyTaskSummary(userID uint, date time.Time) (*models.DailyTaskSummary, error) {
	...
	summary := &models.DailyTaskSummary{
		...
		TotalXPPossible: models.GetTotalPossibleXP(),
		Tasks:           tasks,
...
func (r *dailyTaskRepository) GetTaskHistory(userID uint, limit, offset int) ([]models.DailyTaskSummary, int64, error) {
	...
	var summaries []models.DailyTaskSummary
