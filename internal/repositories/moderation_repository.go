...
func (r *ModerationRepository) CreateContentFlag(flag *models.ContentFlag) error {
	return r.db.Create(flag).Error
}

func (r *ModerationRepository) GetContentFlagByID(id uint) (*models.ContentFlag, error) {
	var flag models.ContentFlag
	err := r.db.Preload("FlaggedBy").Preload("ResolvedBy").First(&flag, id).Error
	return &flag, err
}

func (r *ModerationRepository) GetContentFlags(contentType string, contentID uint) ([]models.ContentFlag, error) {
...
func (r *ModerationRepository) GetUnresolvedFlags(page, limit int) ([]models.ContentFlag, int64, error) {
	var flags []models.ContentFlag
	var total int64

	query := r.db.Model(&models.ContentFlag{}).Where("is_resolved = ?", false)
	query.Count(&total)
...
func (r *ModerationRepository) ResolveFlag(id, resolvedByID uint, notes string) error {
	now := time.Now()
	return r.db.Model(&models.ContentFlag{}).Where("id = ?", id).Updates(map[string]interface{}{
		"is_resolved":      true,
...
func (r *ModerationRepository) DeleteContentFlag(id uint) error {
	return r.db.Delete(&models.ContentFlag{}, id).Error
}
...
func (r *ModerationRepository) CreateReport(report *models.UserReport) error {
	return r.db.Create(report).Error
}

func (r *ModerationRepository) GetReportByID(id uint) (*models.UserReport, error) {
	var report models.UserReport
	err := r.db.Preload("Reporter").Preload("ReportedUser").Preload("HandledBy").First(&report, id).Error
	return &report, err
}

func (r *ModerationRepository) GetReports(status, reportType, reason string, page, limit int) ([]models.UserReport, int64, error) {
...
func (r *ModerationRepository) GetPendingReportsCount() (int64, error) {
	var count int64
	err := r.db.Model(&models.UserReport{}).Where("status = ?", models.ReportStatusPending).Count(&count).Error
	return count, err
}

func (r *ModerationRepository) UpdateReport(report *models.UserReport) error {
...
func (r *ModerationRepository) CheckDuplicateReport(reporterID uint, reportType string, contentID, userID *uint) (bool, error) {
	...
	query := r.db.Model(&models.UserReport{}).
		Where("reporter_id = ? AND report_type = ? AND status IN (?, ?)",
			reporterID, reportType, models.ReportStatusPending, models.ReportStatusReviewing)
...
func (r *ModerationRepository) GetResolvedReportsTodayCount(moderatorID *uint) (int64, error) {
	...
	query := r.db.Model(&models.UserReport{}).
		Where("status = ? AND handled_at >= ?", models.ReportStatusResolved, today)
...
func (r *ModerationRepository) CreateBlock(block *models.UserBlock) error {
	return r.db.Create(block).Error
}

func (r *ModerationRepository) GetBlockByPair(blockerID, blockedID uint) (*models.UserBlock, error) {
	var block models.UserBlock
	err := r.db.Where("blocker_id = ? AND blocked_id = ?", blockerID, blockedID).First(&block).Error
	return &block, err
}

func (r *ModerationRepository) GetBlocksByBlocker(blockerID uint) ([]models.UserBlock, error) {
...
func (r *ModerationRepository) GetBlockedUserIDs(blockerID uint) ([]uint, error) {
	var ids []uint
	err := r.db.Model(&models.UserBlock{}).Where("blocker_id = ?", blockerID).Pluck("blocked_id", &ids).Error
	return ids, err
}
...
func (r *ModerationRepository) DeleteBlock(blockerID, blockedID uint) error {
	return r.db.Where("blocker_id = ? AND blocked_id = ?", blockerID, blockedID).Delete(&models.UserBlock{}).Error
}

func (r *ModerationRepository) GetBlocksCount(userID uint) (int64, error) {
	var count int64
	err := r.db.Model(&models.UserBlock{}).Where("blocker_id = ?", userID).Count(&count).Error
	return count, err
}
...
func (r *ModerationRepository) CreateStrike(strike *models.UserStrike) error {
	return r.db.Create(strike).Error
}

func (r *ModerationRepository) GetStrikeByID(id uint) (*models.UserStrike, error) {
	var strike models.UserStrike
	err := r.db.Preload("User").Preload("IssuedBy").First(&strike, id).Error
	return &strike, err
}

func (r *ModerationRepository) GetUserStrikes(userID uint, activeOnly bool) ([]models.UserStrike, error) {
...
func (r *ModerationRepository) GetActiveStrikesCount(userID uint) (int64, error) {
	var count int64
	err := r.db.Model(&models.UserStrike{}).
...
func (r *ModerationRepository) DeactivateStrike(id uint) error {
	return r.db.Model(&models.UserStrike{}).Where("id = ?", id).Update("is_active", false).Error
}

func (r *ModerationRepository) DeactivateExpiredStrikes() error {
	return r.db.Model(&models.UserStrike{}).
...
func (r *ModerationRepository) CreateModeratorAction(action *models.ModeratorAction) error {
	return r.db.Create(action).Error
}

func (r *ModerationRepository) GetModeratorActions(moderatorID *uint, actionType, targetType string, page, limit int) ([]models.ModeratorAction, int64, error) {
	var actions []models.ModeratorAction
	var total int64

	query := r.db.Model(&models.ModeratorAction{})
	if moderatorID != nil {
...
func (r *ModerationRepository) GetActiveCrisisKeywords(language string) ([]models.CrisisKeyword, error) {
	var keywords []models.CrisisKeyword
...
func (r *ModerationRepository) CreateCrisisKeyword(keyword *models.CrisisKeyword) error {
	return r.db.Create(keyword).Error
}

func (r *ModerationRepository) UpdateCrisisKeyword(keyword *models.CrisisKeyword) error {
	return r.db.Save(keyword).Error
}

func (r *ModerationRepository) DeleteCrisisKeyword(id uint) error {
	return r.db.Delete(&models.CrisisKeyword{}, id).Error
}

func (r *ModerationRepository) GetAllCrisisKeywords() ([]models.CrisisKeyword, error) {
	var keywords []models.CrisisKeyword
...
func (r *ModerationRepository) GetArticlesPendingModeration(status string, page, limit int) ([]models.Article, int64, error) {
	var articles []models.Article
	var total int64

	query := r.db.Model(&models.Article{}).Where("is_user_generated = ?", true)
	if status != "" {
		query = query.Where("moderation_status = ?", status)
	} else {
		query = query.Where("moderation_status IN (?, ?)",
			models.ArticleModerationPending, models.ArticleModerationFlagged)
	}
...
func (r *ModerationRepository) UpdateArticleModeration(articleID uint, status models.ArticleModerationStatus, notes string, moderatorID uint, triggerWarnings []string) error {
	now := time.Now()
	...
	if len(triggerWarnings) > 0 {
		updates["trigger_warnings"] = models.TriggerWarnings(triggerWarnings)
	}

	// If approved, also publish
	if status == models.ArticleModerationApproved {
		updates["status"] = models.ArticleStatusPublished
	}
	// If rejected, set to draft
	if status == models.ArticleModerationRejected {
		updates["status"] = models.ArticleStatusDraft
	}

	return r.db.Model(&models.Article{}).Where("id = ?", articleID).Updates(updates).Error
}

func (r *ModerationRepository) GetPendingArticlesCount() (int64, error) {
	var count int64
	err := r.db.Model(&models.Article{}).
		Where("is_user_generated = ? AND moderation_status = ?", true, models.ArticleModerationPending).
		Count(&count).Error
	return count, err
}
...
func (r *ModerationRepository) GetFlaggedArticlesCount() (int64, error) {
	var count int64
	err := r.db.Model(&models.Article{}).
		Where("is_user_generated = ? AND moderation_status = ?", true, models.ArticleModerationFlagged).
		Count(&count).Error
	return count, err
}
...
func (r *ModerationRepository) GetBannedUsersCount() (int64, error) {
	var count int64
	err := r.db.Model(&models.User{}).Where("is_banned = ?", true).Count(&count).Error
	return count, err
}
...
func (r *ModerationRepository) SetContentWarningPreference(userID uint, preference models.ContentWarningPreference) error {
	return r.db.Model(&models.User{}).Where("id = ?", userID).
...
func (r *ModerationRepository) UpdateForumTriggerWarnings(forumID uint, warnings []string) error {
	return r.db.Model(&models.Forum{}).Where("id = ?", forumID).
		Update("trigger_warnings", models.TriggerWarnings(warnings)).Error
}

func (r *ModerationRepository) FlagForum(forumID uint, reason string) error {
	return r.db.Model(&models.Forum{}).Where("id = ?", forumID).Updates(map[string]interface{}{
		"is_flagged":     true,
...
func (r *ModerationRepository) FlagForumPost(postID uint, reason string) error {
	return r.db.Model(&models.ForumPost{}).Where("id = ?", postID).Updates(map[string]interface{}{
		"is_flagged":     true,
		"flagged_reason": reason,
	}).Error
}
