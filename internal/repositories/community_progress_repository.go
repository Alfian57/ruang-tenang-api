...
func (r *CommunityProgressRepository) GetCommunityStats() (*models.CommunityStats, error) {
	var stats models.CommunityStats
	err := r.db.First(&stats).Error
	if err == gorm.ErrRecordNotFound {
		// Return empty stats if none exist
		return &models.CommunityStats{}, nil
	}
	if err != nil {
		return nil, err
	}
...
func (r *CommunityProgressRepository) UpdateCommunityStats(stats *models.CommunityStats) error {
	// Upsert - create if not exists, update if exists
	var existing models.CommunityStats
...
func (r *CommunityProgressRepository) RecalculateCommunityStats() (*models.CommunityStats, error) {
	now := time.Now()
	month := int(now.Month())
	year := now.Year()
...
func (r *CommunityProgressRepository) GetUsersInLevelRange(minLevel, maxLevel int, limit int) ([]models.User, error) {
	var users []models.User

	// Get min/max exp for the level range
	var minExp, maxExp int64
	r.db.Model(&models.LevelConfig{}).Where("level = ?", minLevel).Select("min_exp").Scan(&minExp)

	if maxLevel >= 10 {
		maxExp = 999999999 // Very high number for max level
	} else {
		r.db.Model(&models.LevelConfig{}).Where("level = ?", maxLevel+1).Select("min_exp").Scan(&maxExp)
	}
...
func (r *CommunityProgressRepository) GetTopUsersInLevel(level int, limit int) ([]models.User, error) {
	var users []models.User
	...
	r.db.Model(&models.LevelConfig{}).Where("level = ?", level).Select("min_exp").Scan(&minExp)
	r.db.Model(&models.LevelConfig{}).Where("level = ?", level+1).Select("min_exp").Scan(&maxExp)
...
func (r *CommunityProgressRepository) GetHallOfFame(month, year int, category string) ([]models.MonthlyHallOfFame, error) {
	var entries []models.MonthlyHallOfFame
...
func (r *CommunityProgressRepository) CreateHallOfFameEntry(entry *models.MonthlyHallOfFame) error {
