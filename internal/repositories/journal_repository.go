...
func (r *JournalRepository) FindByIDAndUserID(id, userID uint) (*models.Journal, error) {
	var journal models.Journal
...
func (r *JournalRepository) Delete(id, userID uint) error {
	return r.db.Where("id = ? AND user_id = ?", id, userID).Delete(&models.Journal{}).Error
}

// FindByUserID finds all journals for a user with pagination
func (r *JournalRepository) FindByUserID(userID uint, page, limit int, tags []string, startDate, endDate *time.Time) ([]models.Journal, int64, error) {
	var journals []models.Journal
	var total int64

	query := r.db.Model(&models.Journal{}).Where("user_id = ?", userID)
...
func (r *JournalRepository) SearchByContent(userID uint, query string, limit int) ([]models.Journal, error) {
	var journals []models.Journal
...
func (r *JournalRepository) FindForAIContext(userID uint, daysBack, maxEntries int) ([]models.Journal, error) {
	var journals []models.Journal
...
func (r *JournalRepository) FindRelevantForAIContext(userID uint, query string, maxEntries int) ([]models.Journal, error) {
	var journals []models.Journal
...
func (r *JournalRepository) UpdateAIAccessedAt(id uint) error {
	now := time.Now()
	return r.db.Model(&models.Journal{}).Where("id = ?", id).Update("ai_accessed_at", now).Error
}

// CountByUserID counts total journals for a user
func (r *JournalRepository) CountByUserID(userID uint) (int64, error) {
	var count int64
	err := r.db.Model(&models.Journal{}).Where("user_id = ?", userID).Count(&count).Error
	return count, err
}
...
func (r *JournalRepository) CountSharedWithAI(userID uint) (int64, error) {
	var count int64
	err := r.db.Model(&models.Journal{}).Where("user_id = ? AND share_with_ai = ?", userID, true).Count(&count).Error
	return count, err
}
...
func (r *JournalSettingsRepository) FindByUserID(userID uint) (*models.JournalSettings, error) {
	var settings models.JournalSettings
...
func (r *JournalSettingsRepository) FindOrCreate(userID uint) (*models.JournalSettings, error) {
	var settings models.JournalSettings
	err := r.db.Where("user_id = ?", userID).First(&settings).Error
	if err == gorm.ErrRecordNotFound {
		settings = models.JournalSettings{
			UserID:              userID,
			AllowAIAccess:       false,
			AIContextDays:       7,
...
func (r *JournalSettingsRepository) Update(settings *models.JournalSettings) error {
...
func (r *JournalAIAccessLogRepository) Create(log *models.JournalAIAccessLog) error {
	return r.db.Create(log).Error
}

// FindByUserID finds AI access logs for a user
func (r *JournalAIAccessLogRepository) FindByUserID(userID uint, limit int) ([]models.JournalAIAccessLog, error) {
	var logs []models.JournalAIAccessLog
...
func (r *JournalAIAccessLogRepository) FindByJournalID(journalID uint) ([]models.JournalAIAccessLog, error) {
	var logs []models.JournalAIAccessLog
