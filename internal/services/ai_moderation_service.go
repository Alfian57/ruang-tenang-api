...
func NewAIModerationService(moderationRepo *repositories.ModerationRepository, cfg *config.Config) *AIModerationService {
...
func (s *AIModerationService) ModerateArticle(title, content string) (*dto.AIModerationResult, error) {
	if s.genaiModel == nil {
		...
		return &dto.AIModerationResult{
			Status:     models.ArticleModerationApproved,
			Confidence: 0,
...
Jika konten mengandung topik sensitif tapi dibahas dengan cara yang supportive dan educational, set status ke "approved" tapi isi trigger_warnings yang sesuai.`, title, content)
	...
	if err != nil {
		...
		return &dto.AIModerationResult{
			Status:     models.ArticleModerationFlagged,
			Confidence: 0,
	...
	var status models.ArticleModerationStatus
	switch result.Status {
	case "approved":
		status = models.ArticleModerationApproved
	case "rejected":
		status = models.ArticleModerationRejected
	default:
		status = models.ArticleModerationFlagged
	}
...
func (s *AIModerationService) DetectCrisis(message string) (*models.CrisisDetectionResult, error) {
	keywords, err := s.moderationRepo.GetActiveCrisisKeywords("id")
	if err != nil {
		return &models.CrisisDetectionResult{IsCrisis: false}, nil
	}
...
func (s *AIModerationService) generateCrisisResponse(category models.CrisisCategory, severity models.CrisisSeverity) string {
