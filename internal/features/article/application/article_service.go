...
type ArticleService struct {
	...
	categoryRepo          *infrastructure.ArticleCategoryRepository
...
func (s *ArticleService) GetPublishedArticleByID(ctx context.Context, id uint) (*dto.ArticleDTO, error) {
	...
	modelArticle := &model.Article{
		...
		ModerationStatus: model.ArticleModerationStatus(article.ModerationStatus),
		IsUserGenerated:  article.IsUserGenerated,
	}

	if !modelArticle.IsPublic() {
...
func (s *ArticleService) GetCategories(ctx context.Context) ([]dto.ArticleCategoryDTO, error) {
	...
	if s.cacheService != nil {
		if cached := s.cacheService.Get(cache.CacheKeyArticleCategories); cached != nil {
	...
	// Store in cache
	if s.cacheService != nil {
		s.cacheService.SetWithTTL(cache.CacheKeyArticleCategories, result, s.cacheService.CategoryTTL)
	}
	return result, nil
}
