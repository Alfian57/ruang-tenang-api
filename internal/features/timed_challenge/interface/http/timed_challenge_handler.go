...
import (
	...
	"github.com/Alfian57/ruang-tenang-api/internal/features/timed_challenge/application")

type TimedChallengeHandler struct {
	challengeService *application.TimedChallengeService
}

func NewTimedChallengeHandler(challengeService *application.TimedChallengeService) *TimedChallengeHandler {
...
func (h *TimedChallengeHandler) StartChallenge(c *gin.Context) {
	...
	if err != nil {
		switch err {
		case application.ErrAlreadyHasActiveChallenge:
			c.JSON(http.StatusConflict, dto.ErrorResponseWithCode(dto.ErrCodeConflict, err.Error()))
		case application.ErrTimedChallengeTemplateNotFound:
...
func (h *TimedChallengeHandler) GetActiveChallenge(c *gin.Context) {
	...
	if err != nil {
		switch err {
		case application.ErrTimedChallengeNotFound:
			c.JSON(http.StatusNotFound, dto.ErrorResponseWithCode(dto.ErrCodeNotFound, err.Error()))
		default:
			c.JSON(http.StatusInternalServerError, dto.ErrorResponse(err.Error()))
		}
