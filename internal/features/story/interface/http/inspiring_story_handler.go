...
import (
	...
	"github.com/Alfian57/ruang-tenang-api/internal/features/story/application")

type InspiringStoryHandler struct {
	storyService *application.InspiringStoryService
}

func NewInspiringStoryHandler(storyService *application.InspiringStoryService) *InspiringStoryHandler {
...
func (h *InspiringStoryHandler) CreateStory(c *gin.Context) {
	...
	if err != nil {
		if se, ok := serviceerror.IsServiceError(err); ok {
