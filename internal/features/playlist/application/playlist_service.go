...
type PlaylistService struct {
	playlistRepo     *infrastructure.PlaylistRepository
	playlistItemRepo *infrastructure.PlaylistItemRepository
...
func (s *PlaylistService) GetPlaylist(ctx context.Context, playlistID, userID uint) (*dto.PlaylistDTO, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, serviceerror.ErrNotFound
		}
		return nil, err
	}
	...
	if playlist.UserID != userID && !playlist.IsPublic {
		return nil, serviceerror.ErrForbidden
	}
...
func (s *PlaylistService) DeletePlaylist(ctx context.Context, playlistID, userID uint) error {
	...
	// Check ownership
	if playlist.UserID != userID {
		return serviceerror.ErrForbidden
	}
