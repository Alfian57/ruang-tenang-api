...
import (
	...
	"github.com/Alfian57/ruang-tenang-api/internal/features/streak_society/application")

type StreakSocietyHandler struct {
	societyService *application.StreakSocietyService
}

func NewStreakSocietyHandler(societyService *application.StreakSocietyService) *StreakSocietyHandler {
...
func (h *StreakSocietyHandler) JoinSociety(c *gin.Context) {
	...
	if err != nil {
		switch err {
		case application.ErrStreakTooLow:
			c.JSON(http.StatusBadRequest, dto.ErrorResponse(err.Error()))
		case application.ErrAlreadySocietyMember:
