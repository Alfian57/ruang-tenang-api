...
func SeedChatSessions(db *gorm.DB) error {
	// Get a test user
	var user model.User
	if err := db.Where("email = ?", "gading@gmail.com").First(&user).Error; err != nil {
	...
	chatData := []struct {
	...
	}{
		{
			...
			Messages: []struct {
				...
				{Role: model.ChatRoleUser, Content: "Saya merasa cemas hari ini."},
				{Role: model.ChatRoleAI, Content: "Saya mengerti perasaan Anda. Rasa cemas adalah hal yang wajar dialami. Cobalah teknik pernapasan 4-7-8 untuk menenangkan diri sejenak. Tarik napas selama 4 detik, tahan 7 detik, lalu hembuskan selama 8 detik."},
				{Role: model.ChatRoleUser, Content: "Bagaimana kalau rasa cemasnya tidak hilang-hilang?"},
				{Role: model.ChatRoleAI, Content: "Jika kecemasan berlangsung terus-menerus dan mengganggu aktivitas sehari-hari, ada beberapa hal yang bisa Anda coba:\n\n1. Journaling - tulis perasaan Anda\n2. Olahraga ringan seperti jalan kaki\n3. Batasi kafein dan alkohol\n4. Bicara dengan orang yang Anda percaya\n\nJika masih berlanjut, pertimbangkan untuk berkonsultasi dengan profesional kesehatan mental. Apakah ada hal spesifik yang membuat Anda cemas?"},
			},
		},
		{
			...
			Messages: []struct {
				...
				{Role: model.ChatRoleUser, Content: "Pekerjaan saya sangat menumpuk dan saya merasa overwhelmed."},
				{Role: model.ChatRoleAI, Content: "Terdengar melelahkan. Beban kerja yang berlebihan bisa sangat menguras energi mental dan fisik. Apakah Anda sudah mencoba memprioritaskan tugas-tugas tersebut? Kadang memecah pekerjaan besar menjadi langkah-langkah kecil bisa membantu."},
				{Role: model.ChatRoleUser, Content: "Sudah coba tapi tetap saja rasanya nggak ada habisnya."},
				{Role: model.ChatRoleAI, Content: "Saya mengerti frustasi Anda. Beberapa tips yang mungkin membantu:\n\n1. Gunakan teknik Pomodoro (kerja 25 menit, istirahat 5 menit)\n2. Delegasikan tugas jika memungkinkan\n3. Komunikasikan beban kerja Anda ke atasan\n4. Tetapkan batasan yang jelas - kerja di jam kerja saja\n\nYang terpenting, jangan lupa untuk beristirahat. Burnout adalah masalah serius. Apakah ada waktu akhir-akhir ini Anda mengambil cuti?"},
			},
		},
		{
